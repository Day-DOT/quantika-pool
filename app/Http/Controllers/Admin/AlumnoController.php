...
class AlumnoController extends Controller
{
    ...
    public function store(StoreAlumnoRequest $request): RedirectResponse
    {
        ...
        $ineFrente = $request->file('ine_tutor_frente');
        $ineReverso = $request->file('ine_tutor_reverso');
        $rutasDocumentos = [
            ...
            'ine_tutor_path' => $ineFrente?->store('alumnos/documentos', 'public'),
            'ine_tutor_path_2' => $ineReverso?->store('alumnos/documentos', 'public'),
        ...
        if ($resultado['cuentaPendienteActivar']) {
            ...
            $quien = $tutorEmail === $resultado['alumno']->email ? 'El alumno' : 'El tutor';
            $mensaje .= " {$quien} ({$tutorEmail}) debe crear su acceso en la pantalla \"Crear cuenta\" del login, usando ese correo y los datos del alumno.";
        }
    ...
    public function update(UpdateAlumnoRequest $request, Alumno $alumno): RedirectResponse
    {
        ...
        if ($tieneTutor) {
            foreach ([
                [$ineFrente, 'ine_tutor_path', $alumno->ine_tutor_path],
                [$ineReverso, 'ine_tutor_path_2', $alumno->ine_tutor_path_2],
            ] as [$archivo, $columna, $rutaAnterior]) {
                if (! $archivo) {
                    continue;
                }
                ...
                $rutasDocumentos[$columna] = $archivo->store('alumnos/documentos', 'public');
            }
        }
